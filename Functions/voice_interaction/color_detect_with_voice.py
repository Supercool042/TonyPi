...
'''
    ...
    对应教程文档路径：  1 教程资料\10.语音交互课程\11.3 颜色识别播报
'''
    ...
    while True:
        ...
        if __isRunning:
            ...
            if detect_color != 'None':
                if detect_color == 'red':
                    speech.play_audio(os.path.join(audio_path, 'red.wav'), volume=70)
                elif detect_color == 'green':
                    speech.play_audio(os.path.join(audio_path, 'green.wav'), volume=70)
                elif detect_color == 'blue':
                    speech.play_audio(os.path.join(audio_path, 'blue.wav'), volume=70)
                time.sleep(2)
                detect_color = 'None'
                draw_color = range_rgb["black"]
            else:
