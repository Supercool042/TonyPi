...
'''
    ...
    运行效果：  程序启动之后，TonyPi 机器人的头部会朝下，并反馈“我准备好了”的语音信息。此时，
                对 TonyPi 进行唤醒（说出唤醒词“小幻小幻”）。等播报“我在”后，可以向机器人下达搬运指令。
                例如，我们说“搬运红色”，机器人接收后会反馈“收到，开始找红色”的语音信息，然后开始寻找并搬运红色物品，
                并放置到对应的 AprilTag 区域

    对应教程文档路径：  1 教程资料\9.AI视觉项目课程
'''
            ...
            if object_center_x == -3:  # -3表示放置阶段，且没有找到目标apriltag，但是找到其他apriltag(-3 indicates the placement phase, and the target AprilTag was not found, but other AprilTags were detected)
                ...
                if x_dis != servo_data['servo2'] and not haved_find_tag:
                    ...
                    if find_box:
                        ...
                        speech.play_audio(os.path.join(audio_path, 'transport_finish.wav'), volume=70)
                        lock_servos = ''
                        step = 6
                        object_color = None
...
cmd_dict = {b"\xaa\x55\x03\x00\xfb": 'wakeup',
            ...
            b"\xaa\x55\x00\x05\xfb": 'transport_red',
            b"\xaa\x55\x00\x06\xfb": 'transport_green',
            b"\xaa\x55\x00\x07\xfb": 'transport_blue'
            }
